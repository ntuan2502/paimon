...
export default function Emojis2Page({ posts }) {
  return (
    <div>
      <Head>
        <title>Genshin | Emojis2</title>
      </Head>
      <div className="m-10 flex justify-center">
        <a
          href="/zip/emojis2.zip"
        ...
        {posts.map((post, index) => (
          <div className="relative" key={index}>
            <img
              ...
              src={`/img/emojis/emojis2/${post.filename}`}
              alt={post.filename}
            />
            <a
              href={`/img/emojis/emojis2/${post.filename}`}
              target="_blank"
...
export async function getServerSideProps(ctx) {
  const postsDirectory = path.join(process.cwd(), "/public/img/emojis/emojis2");
