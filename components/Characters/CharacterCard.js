...
function CharacterCard({ characterName, localeGenshinData }) {
  ...
  return (
    <Link href={`/characters/${data.id}`}>
      <div className="relative bg-white py-6 px-6 rounded-3xl w-64 my-4 hover:shadow-xl cursor-pointer">
        <div
          className={`text-white flex items-center absolute rounded-full py-4 px-4 hover:shadow-xl ${
            data.rarity == 5 ? "bg-orange-400" : "bg-pink-400"
          } left-4 -top-6`}
        >
          {/* <svg
          xmlns="http://www.w3.org/2000/svg"
        ...
        >
          ...
          <img
            className="w-10 h-10"
            src={`/img/character/avatar/${data.id}.png`}
          />
        </div>
        <div className="mt-8">
